...
namespace simple_Shell
{
    class Directory : DirectoryEntry
    {
        ...
        public void deleteDirectory()
        {
            if (this.firs_cluster != 0) 
            {
                int currentCluster = this.firs_cluster;
                int nextCluster = FAT.getClusterNext(currentCluster);

                do
                {
                    FAT.setClusterNext(currentCluster, 0); 
                    currentCluster = nextCluster; 
                    if (currentCluster != -1)
                        nextCluster = FAT.getClusterNext(currentCluster);
